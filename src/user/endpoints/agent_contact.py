...
from src.user.services import UserService, AgentContactService
from src.user.schemas import GetUserSchema, UpdateAgentContactSchema

router = APIRouter()


@router.patch(
    path="/agent-contact", response_model=GetUserSchema, tags=["User: Profile"]
)
@inject
